...
describe('UsersController', () => {
  ...
  it('create returns created user', async () => {
    ...
    await expect(controller.create({} as any)).resolves.toEqual({ email: 'b@x.com' });
  });
  ...
  it('update allows owner', async () => {
    ...
    await expect(controller.update('1', { name: 'Updated' } as any, req as any)).resolves.toEqual({ name: 'Updated' });
  });

  it('findOne returns user when found', async () => {
    mockService.findById.mockResolvedValue({ _id: '1', email: 'u@x.com' });
    await expect(controller.findOne('1')).resolves.toEqual({ _id: '1', email: 'u@x.com' });
  });

  it('update denies non-owner non-admin', async () => {
    const req = { user: { sub: '2', role: Role.User } };
    await expect(controller.update('1', { name: 'Updated' } as any, req as any)).rejects.toThrow(ForbiddenException);
  });
  ...
  it('admin can delete any user', async () => {
    ...
    await expect(controller.remove('1', req as any)).resolves.toEqual({ deleted: true });
  });

  it('remove denies non-owner non-admin', async () => {
    const req = { user: { sub: '2', role: Role.User } };
    await expect(controller.remove('1', req as any)).rejects.toThrow(ForbiddenException);
  });
});
